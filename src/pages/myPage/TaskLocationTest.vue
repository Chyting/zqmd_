...
export default defineComponent({
  ...
  setup() {
    const [setIsNative] = useState(false);
    const log = ref('');
    ...
    onMounted(async () => {
      ...
      const retNoEncryption = await taskLocationTest(log, sqlite, platform);
      if (!retNoEncryption) {
        log.value = log.value.concat('* taskLocationTest failed*\n');
    ...
    return { log, setIsNative };
  },
});
async function taskLocationTest(
  log: Ref<string>,
  sqlite: SQLiteHook,
  platform: string
) {
  try {
    ...
    // create a connection for TaskLocation
    const db = await sqlite.createConnection('TaskLocation');

    log.value = log.value.concat(
      '> createConnection ' + " 'TaskLocation' successful\n"
    );
    // open TaskLocation database
    await db.open();
    log.value = log.value.concat("> open 'TaskLocation' successful\n");
    const createTablesTaskLocation = `
    ...
    `;
    const dropTablesTablesTaskLocation = `
    ...
    res = await db.execute(dropTablesTablesTaskLocation, false);
    ...
    // Create tables
    res = await db.execute(createTablesTaskLocation);
